/*
    Hoisting is the process of virtually moving the variable or function definition to the beginning of 
    the scope, usually for variable statement var and function declaration function fun() {...}

    When let (and also const and class, which have similar declaration behavior as let) declarations were
    introduced by ES2015, many developers including myself were using the hoisting definition to describe 
    how variables are accessed. But after more search on the question, surprisingly for me hoisting is not 
    the correct term to describe the initialization and availability of the let varirables.
...
/*
    ...
    As it turns out, the possibility to first use and then declare a variable or function creates 
    confusion. Suppose you scroll a big file and suddenly see and undeclared variable, how the hell it 
    does appear here and where it is defined?
    
    Of course a practiced JavaScript developer won't code this way. But in the thousands of JavaScript 
    GitHub repos is quite possible to deal with such code.

    Even looking at the code sample presented above, it is difficult to understand the declaration flow 
    in the code. Naturally, first you declare or describe an unknown term. And only later make phrases 
    with it. let encourages you to follow this approach with variables.
*/
...
/*
    ...
    When the engine works with variables, their lifecycle consists of the following phases:
        ...
        II.  Initialization phase is allocating memory and creating a binding for the variable in the 
             scope. At this step the variable is automatically initialized with undefined.
        III. Assignment phase is assigning a value to the initialized variable.
    
    A variable has uninitialzed state when it passed the declaration phase, yet didn't reach the 
    initilization.

    Notice that in terms of variable lifecycle, declaration phase is a different term than generally 
    speaking variable declaration. In simple words, the engine processes the variable declaration in 3 
    phases:
...
/*
    ...
    After declaration and initialization, but before assignment phase, the variable has undefined value 
    and can be used already.
...
/*
    When JavaScript starts executing multiplyByTen(4) and enters the function scope, the variable ten 
    passes declaration and initialization steps, before the first statement. So when calling 
    console.log(ten), it is logged undefined. The statement ten = 10 assigns an initial value. After 
    assignment, the line console.log(ten) logs correctly 10 value.
*/
...
/*
    ...
    function scope (only one step). funName() can be invoked in any place of the scope, not depending on 
    the declaration statement position (it can be even at the end).
...
/*
    ...
    let variables are processed differently than var. The main distinction is that declaration and 
    initialization phases are splited.
    ...
    statement. Immediately the variable passes the declaration phase, registering its name in the scope 
    (step 1). The interpreter continues parsing the block statements line by line.

    If you try to access variable at this stage, JavaScript will throw ReferenceError: variable is not 
    defined. It happens because the variable state is uninitialized. variable is in the temporal dead zone.

    When interpreter reaches the statement let variable, the initilization phase is passed (step 2). Now 
    the variable state is initialized and accessing it evaluates to undefined. The variable exits the 
    temporal dead zone.
...
/*
    ...
    Because number has uninitialized state and is in a temporal dead zone, an attempt to access the 
    variable throws ReferenceError: number is not defined. Later the statement let number makes the 
    initialization. Now the variable can be accessed, but its value is undefined. The assignment statement 
    number = 5 of course makes the assignment phase.
...
/*
    ...
    As mentioned above, hoisting is variable's coupled declaration and initialization at the top of the 
    scope. let lifecycle however decouples declaration and initialization phases. Decoupling vanishes the 
    hoisting term for let.
...
/*
    ...
    Because the declaration and initialization phases are decoupled, hoisting is not valid for a let 
    variable (including for const and class). Before initialization, the variable is in temporal dead zone 
    ...
    To keep variables declaration smooth, these tips are recommended:
        ...
        II. Keep the variables as hidden as possible. The less variables are exposed, the more modular 
            your code becomes.
*/
...
/*
    The difference between thisIsHoisted() and thisIsNotHoisted() function is that thisIsNotHoisted is 
    declared as const so, it can not be changed. Whereas, thisIsHoisted() is declared as function and can 
    be changed.
*/
...
/*
    ...
    The difference between the function error of const and var is that
        When const or let is used, the variable is not declared at all. The variable does not exists at 
        all. This is not hoisted and JavaScript does not know what that means.

        But in case of var, it hoists the declaration of the variable. So, it creates a variable called
        thisIsNotHoistedVar. However, it does not assign it a value until the code is executed. So, 
        function thisIsNotHoistedVar exists. It's just equal to undefined. So, when it is invoked like a 
        function, interpreter throws an error saying this is an undefined variable and it can't be invoked 
        like a function, this is a TypeError.
    ...
    Then any function definitions just get saved in memory, so if somebody wants to use it they're able 
    to. Variable initializations, if they're lexically scoped they will be declared, but they will not be 
    initialized. Meaning anything declared with var will say this variable exists but it's not going to be 
    set equal to anything until later.

    Second phase
    It is known as execution phase, whereby the code is actually executed. That is when things like const 
    or let get invoked or get both declared and initialized.
*/